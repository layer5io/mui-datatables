...
class TableBody extends React.Component<TableBodyProps> {
  ...
  handleRowSelect = (data: { index: number; dataIndex: number }, event: React.MouseEvent) => {
    ...
    if (shiftKey && previousSelectedRow && previousSelectedRow.index < this.props.data.length) {
      ...
      const clickedDataIndex = this.props.data[data.index]?.dataIndex as number;
      ...
      while (curIndex !== data.index) {
        const dataIndex = this.props.data[curIndex]?.dataIndex as number;
  ...
  processRow = (row: MUIDataTableCellValue[], columnOrder: number[]) => {
    ...
    for (let ii = 0; ii < row.length; ii++) {
      const columnIndex = columnOrder[ii];
      if (columnIndex === undefined) continue;
      ret.push({ value: row[columnIndex], index: columnIndex });
    }
    return ret;
  };
              ...
              <React.Fragment key={rowIndex}>
                <TableBodyRow
                  ...
                  {processedRow.map((column) => {
                    const tableColumn = columns[column.index];
                    if (!tableColumn || tableColumn.display !== 'true') return null;
                    const cellValue = column.value instanceof Date ? String(column.value) : column.value;

                    return (
                      <TableBodyCell
                        {...(tableColumn.setCellProps
                          ? tableColumn.setCellProps(column.value, dataIndex, column.index) || {}
                          : {})}
                        data-testid={`MuiDataTableBodyCell-${column.index}-${rowIndex}`}
                        dataIndex={dataIndex}
                        rowIndex={rowIndex}
                        colIndex={column.index}
                        columnHeader={tableColumn.label}
                        print={tableColumn.print}
                        options={options}
                        tableId={tableId}
                        key={column.index}>
                        {cellValue}
