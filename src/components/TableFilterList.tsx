...
const TableFilterList = ({
  ...
  const removeFilter = (
    ...
    if (Array.isArray(removedFilter) && removedFilter.length === 0) {
      removedFilter = filterList[index] as string | string[];
    }
      ...
      <ItemComponent
        ...
        onDelete={() =>
          removeFilter(
            ...
            columnNames[index]?.name as string,
            type as string,
        ...
        filterProps={
          options.setFilterChipProps
            ? options.setFilterChipProps(index, columnNames[index]?.name as string, item[customFilterItemIndex] || '')
            : {}
        }
      />
    );
  ...
  const filterChip = (index: number, data: string, colIndex: number) => (
    <ItemComponent
      ...
      key={colIndex}
      onDelete={() => removeFilter(index, data, columnNames[index]?.name as string, 'chip')}
      className={classes.chip}
      itemKey={colIndex}
      index={index}
      data={data}
      columnNames={columnNames}
      filterProps={
        options.setFilterChipProps ? options.setFilterChipProps(index, columnNames[index]?.name as string, data) : {}
      }
    />
  );
    ...
    return fl.map((item, index) => {
      if (columnNames[index]?.filterType === 'custom' && fl[index]?.length) {
        ...
        if (Array.isArray(filterListRenderersValue)) {
          ...
          return customFilterChip(filterListRenderersValue as string, index, index, item, false);
        }
      }
