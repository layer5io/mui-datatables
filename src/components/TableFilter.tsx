...
interface TableFilterProps {
  ...
  classes: Record<keyof ReturnType<typeof defaultFilterStyles>, string>;
  customFooter?: (filterList: MUIDataTableFilterList, applyFilters: () => MUIDataTableFilterList) => React.ReactNode;
...
class TableFilter extends React.Component<TableFilterProps, TableFilterState> {
  ...
  filterUpdate = (
    ...
    const newFilterList = this.state.filterList.slice(0);
  ...
  handleDropdownChange = (event: React.ChangeEvent<{ value: unknown }>, index: number, column: string) => {
    const labelFilterAll = this.props.options.textLabels?.filter?.all || '';
    const value = event.target.value === labelFilterAll ? [] : [event.target.value as string];
  ...
  handleMultiselectChange = (index: number, value: string | string[], column: string) => {
  ...
  handleTextFieldChange = (event: React.ChangeEvent<HTMLInputElement>, index: number, column: string) => {
  ...
  handleCustomChange = (value: string | string[], index: number, column: MUIDataTableColumnState) => {
    this.filterUpdate(index, value, column.name, column.filterType as string);
    if (this.props.options.confirmFilters !== true) {
      this.props.onFilterUpdate(index, value, column.name, column.filterType as string);
    }
  };
  ...
  renderCheckbox(
    ...
    const renderItem = filterOpts?.renderValue ? (filterOpts.renderValue as (v: string) => string) : (v: string) => v;
      ...
      <Grid key={index} size={width} className={classes.gridListTile}>
        <FormControl key={index} variant={'standard'} fullWidth>
          ...
          <Select
            ...
            name={column.name}
            onChange={(event) =>
              this.handleDropdownChange(event as unknown as React.ChangeEvent<{ value: unknown }>, index, column.name)
            }
      ...
      <Grid key={index} size={width} className={classes.gridListTile}>
        <FormControl key={index} fullWidth>
          <TextField
            ...
            onChange={(event) =>
              this.handleTextFieldChange(event as React.ChangeEvent<HTMLInputElement>, index, column.name)
            }
          />
        </FormControl>
      </Grid>
      ...
      <Grid key={index} size={width} className={classes.gridListTile}>
        <FormControl key={index} variant={'standard'} fullWidth>
          ...
          <Select
            ...
            renderValue={(selected) => (selected as string[]).map(renderItem).join(', ')}
            name={column.name}
  ...
  renderCustomField(column: MUIDataTableColumnState, index: number) {
    ...
    const width = filterOpts?.fullWidth === true ? 12 : 6;
    const display =
      filterOpts?.display || (options.filterOptions && (options.filterOptions as Record<string, unknown>).display);
    ...
    return (
      <Grid key={index} size={width} className={classes.gridListTile}>
        <FormControl key={index} fullWidth>
          ...
          )(filterList, this.handleCustomChange, index, column, filterData)}
        </FormControl>
      </Grid>
    );
  }
  ...
  resetFilters = () => {
    this.setState({ filterList: this.props.columns.map(() => []) });
    if (this.props.options.confirmFilters !== true) {
  ...
  render() {
    ...
    return (
      <div className={classes.root}>
        ...
        <Grid container direction="row" sx={{ justifyContent: 'flex-start', alignItems: 'center' }} spacing={4}>
