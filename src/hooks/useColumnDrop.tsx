...
const getColModel = (
  ...
  columnOrder.forEach((colIdx) => {
    ...
    if (!(columns[colIdx] && columns[colIdx].display !== 'true')) {
      const previousCol = cmIndx !== -1 ? colModel[cmIndx] : undefined;
      const prevLeft = previousCol ? previousCol.left + previousCol.width : parentOffsetLeft + leftMostCell!.offsetLeft;
      colModel.push({
        left: prevLeft,
...
const handleHover = (opts: HoverOptions): void => {
  ...
  if (hoverIdx !== index) {
    ...
    for (let ii = 0; ii < newColModel.length; ii++) {
      const column = newColModel[ii];
      if (column && newX > column.left && newX < column.left + column.width) {
        modelIdx = column.columnIndex as number;
        break;
      }
    }
    ...
    if (modelIdx === mon.getItem().colIndex) {
      ...
      for (let ii = 0; ii < columnOrder.length; ii++) {
        ...
        for (const elm of elms) {
          elm.style.transition = transitionTime + 'ms';
          elm.style.transform = 'translateX(' + transitions[ii] + 'px)';
          allElms.push(elm);
        }
        ...
        const divider = tableRef
          ...
          : [];
        for (const item of divider) {
          item.style.transition = transitionTime + 'ms';
          item.style.transform = 'translateX(' + transitions[columnOrder[ii] as number] + 'px)';
          dividers.push(item);
        }
      }
